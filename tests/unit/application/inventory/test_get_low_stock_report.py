...
def test_get_low_stock_report_success(fake_uow: FakeUnitOfWork):
    batch_1 = StockBatch(
        init_product_id="prod-1", init_init_quantity=50, 
        init_current_quantity=5, unit_cost=Money(amount=Decimal("10.0"))
    )
    batch_2 = StockBatch(
        init_product_id="prod-2", init_init_quantity=20, 
        init_current_quantity=2, unit_cost=Money(amount=Decimal("15.0"))
    )
    batch_3 = StockBatch(
        init_product_id="prod-3", init_init_quantity=100, 
        init_current_quantity=100, unit_cost=Money(amount=Decimal("5.0"))
    )
