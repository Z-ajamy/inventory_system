...
class GetProductUseCase:
    ...
    def execute(self, product_id: str) -> ProductResponseDTO:
        with self.uow as db:
            ...
            available_qty = db.batches.get_total_quantity_for_product(product_id=product.id)

            attributes = {}
            if isinstance(product, PenProduct):
                attributes = {"color_id": product.color_id, "pen_type_id": product.pen_type_id}
            elif isinstance(product, RulerProduct):
                attributes = {"ruler_type_id": product.ruler_type_id, "length_cm": product.length_cm}
            elif isinstance(product, NoteBook):
                attributes = {"type_id": product.type_id, "page_count": product.page_count}
